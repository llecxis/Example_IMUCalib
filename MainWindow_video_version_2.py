...
class MainWindow(QMainWindow):
    ...
    def calibrate(self, n_time = 400, t_time = 1996):
        if self.state == 1:
            
            self.sld.setValue(n_time)
            self.n_qtr_shift()
            self.sld.setValue(t_time)
            self.t_qtr_shift()
            self.sld.setValue(0)
    ...
    def timerCallback(self): #сюда преобразование координат qweqrty
        ...
        try:
            ...
            if  (self.state == 1) and (self.flag_norm == 1):
                ...
                self.qtr63 = self.rotBnoAtVtk*R.from_quat(self.vm.frame['5563'][self.ti])
                # self.qtr61 = self.rotBnoAtVtk*R.from_quat(self.vm.frame['5561'][self.ti]) # humerus_r
                # self.qtr59 = self.rotBnoAtVtk*R.from_quat(self.vm.frame['5559'][self.ti]) # ulna_l + radius_l
                self.qtr71 = self.rotBnoAtVtk*R.from_quat(self.vm.frame['5571'][self.ti]) # humerus_l
                self.qtr69 = self.rotBnoAtVtk*R.from_quat(self.vm.frame['5569'][self.ti]) # ulna_l + radius_l
                ...
                for el in range(len(self.scene.modelActor)):
                    ...
                    # if (el == 3) :
                    #     temp_qtr_2 = self.ci.calcSegmentAtBody(0,self.rotBox0AtIMU['5561']*self.qtr61,temp_qtr)
                    #     self.motion_flag =  [1, np.array(temp_qtr_2.as_quat())]

                    # if (el == 6) or (el == 11):
                    #     temp_qtr_2 = self.ci.calcSegmentAtBody(0,self.rotBox0AtIMU['5559']*self.qtr59,temp_qtr)
